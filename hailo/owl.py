...
class Owl:
    ...
    def hoot(self):
        ...
        def _create_detector(algo):
            ...
            if algo == 'hailo':
                from utils.hailo_inference import HailoInference
                return HailoInference(
                    model_path=self._model_path,
                    confidence_threshold=self._gog_confidence
                )
            elif algo == 'gog':
        ...
        try:
            while True:
                ...
                if self._detection_enable and weed_detector is not None:
                    ...
                    if algorithm == 'hailo':
                        detections = weed_detector.run(cropped_frame)
                        boxes = []
                        weed_centres = []
                        image_out = cropped_frame.copy() if return_image_out else cropped_frame
                        ...
                        for det in detections:
                            ...
                            if return_image_out:
                                ...
                                cv2.putText(image_out, f"Adventice {conf:.2f}",
                                            (x1, max(y1 - 10, 10)),
